...
test("daily editions are authored, distinct, date-gated and do not silently loop", () => {
  assert.equal(dailyDiscoveries.length, 59);
  assert.equal(new Set(dailyDiscoveries.map((item) => item.slug)).size, 59);
  assert.equal(new Set(dailyDiscoveries.map((item) => item.date)).size, 59);
...
test("release dates are independent from source checks and expire out of the new window", () => {
  ...
  assert.equal(recentGuideReleases(publicGuides, "2026-09-15").length, 56);
