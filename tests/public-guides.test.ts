...
test("guide set contains eighty-four distinct, sourced public experiences, not Selected or demo records", () => {
  assert.equal(publicGuides.length, 84);
  assert.equal(
  ...
  assert.equal(new Set(publicGuides.map((p) => p.countrySlug)).size, 44);
  assert.equal(
