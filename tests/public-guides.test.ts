...
test("guide set contains ninety-eight distinct, sourced public experiences, not Selected or demo records", () => {
  assert.equal(publicGuides.length, 98);
  assert.equal(
  ...
  assert.equal(new Set(publicGuides.map((p) => p.countrySlug)).size, 54);
  assert.equal(
