...
test("guide set contains ninety-two distinct, sourced public experiences, not Selected or demo records", () => {
  assert.equal(publicGuides.length, 92);
  assert.equal(
  ...
  assert.equal(new Set(publicGuides.map((p) => p.countrySlug)).size, 50);
  assert.equal(
