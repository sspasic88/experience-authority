...
test("guide set contains ninety-five distinct, sourced public experiences, not Selected or demo records", () => {
  assert.equal(publicGuides.length, 95);
  assert.equal(
  ...
  assert.equal(new Set(publicGuides.map((p) => p.countrySlug)).size, 52);
  assert.equal(
