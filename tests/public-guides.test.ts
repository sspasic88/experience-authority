...
test("guide set contains fourteen distinct, sourced public experiences, not Selected or demo records", () => {
  assert.equal(publicGuides.length, 14);
  assert.equal(
  ...
  assert.equal(new Set(publicGuides.map((p) => p.countrySlug)).size, 13);
