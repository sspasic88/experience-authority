...
Current authored catalogue: 172 public-source guides across 105 countries and territories, 16 local planning chapters, nine Journal articles, dated daily discoveries and an explicit new-publication log. The public edition serves only the subset that passes the current evidence and review-date gate, so the visible count can be lower when a guide is due for rechecking. See [build status](docs/build-status.md) for the current scope and boundaries.
