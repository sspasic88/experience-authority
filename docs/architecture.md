...
The standard catalogue contains 66 original public-source guides in 37 countries and territories and 11 active fields, connected through 12 editorial collections and ten local planning chapters. A separate opt-in demo mode retains eight fictional concepts. Neither is the 25 locally validated Selected experiences envisaged in the original brief. The current public-source edition is not that validation milestone. Guide cards use real photographs with individual media records, licences or explicit uncleared-reuse status, and depiction boundaries. Every public image exposes one direct source credit in the lower-right corner. Stock and activity-context imagery is explicitly credited, not documentary evidence of an EA visit.
